...
# Step 3: Group by 'Year' and calculate the average for 'popularity', 'influence', 'cc', and 'impulse'
metrics = ['popularity', 'influence', 'cc', 'impulse']
yearly_metrics = df.groupby('Year')[metrics].mean()
...
# Step 5: Plot the results for each metric and save the figures
for metric in metrics:
    plt.figure(figsize=(6, 6))  # Square figure
    plt.plot(yearly_metrics.index, yearly_metrics[metric], marker='o', linestyle='-', color='green', alpha=0.7)
    ...
    plt.xlim(yearly_metrics.index.min(), yearly_metrics.index.max())
    plt.ylim(0, yearly_metrics[metric].max() * 1.1)  # To add some space above the max value
...
# Step 6: For pop_class, inf_class, count values not equal to 'C5' and calculate averages per year
...
# Step 7: Plot the number of papers considered per year
papers_per_year = df.groupby('Year').size()

# Step 8: Create the bar chart for total papers, popular works, and influential works per year
